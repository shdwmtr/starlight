...
function UninstallerSelectComponents() {
  ...
  const [uninstallerState, setUninstalling] = useState<UninstallerStateProps>({
    hasFinished: false,
    inProgress: false,
  });
  const [uninstallList, setUninstallList] = useState<
    UninstallingComponentProps[]
  >([]);

  const StartUninstall = async () => {
    setUninstalling({ hasFinished: false, inProgress: true });

    let bufferUninstallList: UninstallingComponentProps[] = propMap.map(
      (component) => ({
        success: false,
        inProgress: true,
        type: component.type,
    ...
    for (const component of bufferUninstallList) {
      ...
      const result = await invoke("delete_recursive", {
        path: await path.resolve(steamPath, component.path),
      })
        .then((_: any) => ({ success: true }))
        .catch((error: any) => ({ success: false, message: error }));
      console.log(
        "Uninstall result:",
        await path.resolve(steamPath, component.path),
        result,
      );
  ...
  const GetSizeFromType = (type: eUninstallType) => {
    const totalSize = propMap
      ?.filter((component) => component.type === type)
      ?.reduce((total, component) => total + component.size, 0);
    return FormatBytes(totalSize);
  };

  const RenderComponentIsUninstalling = ({
    name,
    type,
  }: {
    name: string;
    ...
    const targetComponent = uninstallList?.filter(
      (component) => component.type === type,
    );

    const hasFinished = targetComponent?.every(
      (component) => !component.inProgress,
    );
    const hasFailed = targetComponent?.some((component) => !component.success);
    const failedMessage = targetComponent?.map(
    ...
    const RenderIsExcluded = () => {
      return (
        <>
          <svg
            className="uninstallFinishedCheck"
          ...
          <Tooltip
            anchorSelect={`#component__${eUninstallType[type]}`}
            place="right-end"
          >
            Component was excluded.
          </Tooltip>

          <p className="isUninstallingProp">
            {name}: {GetSizeFromType(type)}
          </p>
        </>
      );
    };
  ...
  const RenderComponentSize = ({
    ...
    const localPaths = propMap
      ?.filter((component) => component.type === type)
      ?.map((component) => component.path);
    const isChecked = propMap
      ?.filter((component) => component.type === type)
      ?.every((component) => component.checked);
        ...
        <div className="check" id={`component__${eUninstallType[type]}`}>
          {uninstallerState.inProgress || uninstallerState.hasFinished ? (
            ...
            <>
              <CustomCheckbox
                isChecked={isChecked}
                onChange={(_) => HandleComponentChange(type)}
              >
                <p>
                  {name}: {GetSizeFromType(type)}
                </p>
              ...
              >
                {localPaths?.map((path) => (
                  <div key={path}>{`${steamPath}/${path}`}</div>
                ))}
              </Tooltip>
            </>
          )}
  ...
  const CalculateMillenniumSize = async () => {
    ...
    Promise.all(
      targetComponents.map(async (component: any) => ({
        type: component.id,
  ...
  const CalculateSelectedSize = () => {
    return propMap?.reduce(
      (total, component) =>
        component.checked ? total + component.size : total,
      0,
    );
  };

  useEffect(() => {
    CalculateMillenniumSize();
  }, []);
          ...
          <div className="options">
            ...
            >
              ...
              <div className="select-uninstall-components">
                <RenderComponentSize
                  name={"Millennium"}
                  type={eUninstallType.MILLENNIUM}
                />
                <RenderComponentSize
                  name={"Core Components"}
                  type={eUninstallType.CORE}
                />
                <RenderComponentSize
                  name={"Python Dependencies"}
                  type={eUninstallType.PYTHON}
                />
                <RenderComponentSize
                  name={"Themes"}
                  type={eUninstallType.THEMES}
                />
                <RenderComponentSize
                  name={"Plugins"}
                  type={eUninstallType.PLUGINS}
                />
              </div>
        ...
        <FooterLegend>
          {uninstallerState.hasFinished ? (
            ...
            <button
              id="install"
              className={`btn next`}
              onClick={(_) => !uninstallerState.inProgress && StartUninstall()}
            >
