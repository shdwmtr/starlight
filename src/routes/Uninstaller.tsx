...
import {Core, eInstallMessageType, InstallerMessageProps} from '../components/ForeignFunctions';
...
const InstallCoreFinished: React.FC<InstallerFinishedProps> = ({installSuccess, strFailedMessage}) => {
    ...
    const InstallSuccess = () => {
        return (<div className="container" id="install-success">
            <div className='header'>Successfully Removed! Thanks for trying Millennium 💖</div>
            <div className='description'>You're themes and presets haven't been touched 😊. We're sorry to see you go.
            </div>
        </div>)
    }
...
const InstallerInProgress = ({status}: { status: string }) => {
    return (<motion.div transition={{delay: .5,}} initial={{y: "-100vh"}} animate={{y: 0}} className='installer-box'>
...
function UninstallPanel() {
    ...
    return (<>
        ...
        <div className="content-box" id='installer-pane'>
            {isInstalling ? <InstallerInProgress status={strProgressMessage}/> : <>
