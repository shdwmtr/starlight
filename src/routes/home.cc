...
{   
    ImGuiIO& io = GetIO();
    static float DEFAULT_BORDER_COL = GetStyleColorVec4(ImGuiCol_Border).x;

    auto& state = optionStates[props.title];
    state.initPos = GetCursorPosY(); // Current position

    float currentColor = SmoothFloat(props.title, state.initPos, state.isHovered, 5, 0.15f, std::make_tuple(DEFAULT_BORDER_COL, 0.22f));
...
const void RenderHome(float xPos)
{
    ...
    RenderOption({ "Install", "Integrate Millennium into your Steam® Client." }, ContainerWidth, ContainerHeight, ContainerSpacing);
    RenderOption({ "Remove" , "Selectively uninstall portions of Millennium." }, ContainerWidth, ContainerHeight, ContainerSpacing);
